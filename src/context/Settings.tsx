...
function getDefault(): Required<SettingsNew> {
  ...
  // Next.js fallback values safe for server-side evaluation
  const isClient = typeof window !== 'undefined';
  const defaultFontSize = isClient && window.innerWidth > 768 && window.innerHeight > 500 ? '1.2' : '0.8';
...
async function setLanguage(
  ...
  if (!language.endsWith('-GS')) {
    ...
    try {
      // Swapped out VITE_GS_TRANSLATION_URL for Next.js public pattern environment variable shorthand
      const gsUrl = process.env.NEXT_PUBLIC_GS_TRANSLATION_URL || '';
      const promise = fetch(`${gsUrl}?lang=${language.slice(0, -3)}`, {
        credentials: 'omit',
      }).then(res => res.json());
...
export function SettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const settings = useMemo(() => {
    if (typeof window === 'undefined') return resolvedLocal;
